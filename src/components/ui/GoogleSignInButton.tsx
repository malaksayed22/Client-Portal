import { useGoogleLogin } from "@react-oauth/google";
import { motion } from "framer-motion";
import { useState } from "react";

export type GoogleUser = {
  googleId: string;
  name: string;
  email: string;
  picture: string;
};

type GoogleSignInButtonProps = {
  onSuccess: (user: GoogleUser) => void;
  onError?: () => void;
  label: string;
};

export const GoogleSignInButton = ({
  onSuccess,
  onError,
  label,
}: GoogleSignInButtonProps) => {
  const [loading, setLoading] = useState(false);

  const login = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setLoading(true);
      try {
        const res = await fetch(
          "https://www.googleapis.com/oauth2/v3/userinfo",
          {
            headers: {
              Authorization: `Bearer ${tokenResponse.access_token}`,
            },
          },
        );

        if (!res.ok) {
          throw new Error("google-user-info-failed");
        }

        const data = (await res.json()) as {
          sub?: string;
          name?: string;
          email?: string;
          picture?: string;
        };

        if (!data.sub || !data.name || !data.email) {
          throw new Error("google-user-info-missing");
        }

        onSuccess({
          googleId: data.sub,
          name: data.name,
          email: data.email,
          picture: data.picture ?? "",
        });
      } catch {
        onError?.();
      } finally {
        setLoading(false);
      }
    },
    onError: () => {
      onError?.();
      setLoading(false);
    },
  });

  return (
    <motion.button
      type="button"
      dir="rtl"
      whileHover={loading ? undefined : { scale: 1.01 }}
      whileTap={loading ? undefined : { scale: 0.97 }}
      onClick={() => {
        if (!loading) {
          login();
        }
      }}
      disabled={loading}
      className={[
        "h-12 w-full rounded-xl border font-heading text-[15px] font-medium",
        "inline-flex items-center justify-center gap-3",
        "border-[#E5E7EB] bg-white text-textPrimary",
        "hover:bg-[#F9FAFB] hover:border-[#D1D5DB]",
        "dark:border-border dark:bg-background dark:text-textPrimary",
        "disabled:cursor-not-allowed disabled:opacity-70",
      ].join(" ")}
    >
      {loading ? (
        <span className="h-5 w-5 animate-spin rounded-full border-2 border-border border-t-primary" />
      ) : (
        <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26
    1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92
    3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66
    -2.23 1.06-3.71 1.06-2.86 0-5.29-1.93
    -6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35
    -2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45
    1.18 4.93l3.66-2.84z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45
    2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18
    7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
      )}
      <span>{loading ? "جاري الاتصال..." : label}</span>
    </motion.button>
  );
};
